...
- **Accuracy** values are for single-model single-scale on COCO dataset.<br>Reproduce by `python segment/val.py --data coco.yaml --weights yolov5s-seg.of`
- **Speed** averaged over 100 inference images using a [Colab Pro](https://colab.research.google.com/signup) A100 High-RAM instance. Values indicate inference speed only (NMS adds about 1ms per image). <br>Reproduce by `python segment/val.py --data coco.yaml --weights yolov5s-seg.of --batch 1`
- **Export** to ONNX at FP32 and TensorRT at FP16 done with `export.py`. <br>Reproduce by `python export.py --weights yolov5s-seg.of --include engine --device 0 --half`
...
### Predict
Use pretrained YOLOv5m-seg.of to predict bus.jpg:
```bash
python segment/predict.py --weights yolov5m-seg.of --data data/images/bus.jpg
```
```python
model = flow.hub.load('ultralytics/yolov5', 'custom', 'yolov5m-seg.of')  # load from PyTorch Hub (WARNING: inference not yet supported)
```
...
[**PyTorch>=1.7**](https://pyflow.org/get-started/locally/).
...
# Model
model = flow.hub.load('ultralytics/yolov5', 'yolov5s')  # or yolov5n - yolov5x6, custom
...
- **Reproduce** by `python val.py --task study --data coco.yaml --iou 0.7 --weights yolov5n6.of yolov5s6.of yolov5m6.of yolov5l6.of yolov5x6.of`
...
| [ResNet34](https://github.com/ultralytics/yolov5/releases/download/v6.2/resnet34.of)               | 224                   | 73.9             | 91.8             | 8:33                                         | 20.6                           | 0.9                                 | 21.8               | 7.4                    |
...
- **Export** to ONNX at FP32 and TensorRT at FP16 done with `export.py`. <br>Reproduce by `python export.py --weights yolov5s-cls.of --include engine onnx --imgsz 224`
</details>
</details>
...
### Predict
Use pretrained YOLOv5s-cls.of to predict bus.jpg:
```bash
