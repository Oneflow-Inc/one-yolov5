...
Usage - Single-GPU training:
    $ python segment/train.py --data coco128-seg.yaml --weights yolov5s-seg.of --img 640  # from pretrained (recommended)
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg, resume, noval, nosave, workers, freeze, mask_ratio, bbox_iou_optim, multi_tensor_optimizer = (
        Path(opt.save_dir),
        opt.epochs,
        opt.batch_size,
        opt.weights,
    ...
    compute_loss = ComputeLoss(model, overlap=overlap, bbox_iou_optim=bbox_iou_optim)  # init loss class
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _, masks) in pbar:  # batch ------------------------------------------------------
            ...
            if RANK in {-1, 0}:
                ...
                writer.add_scalar("train/box_loss", mloss[0].numpy(), ni)
                writer.add_scalar("train/seg_loss", mloss[1].numpy(), ni)
                writer.add_scalar("train/obj_loss", mloss[2].numpy(), ni)
                writer.add_scalar("train/cls_loss", mloss[3].numpy(), ni)
        ...
        if RANK in {-1, 0}:
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ...
                if opt.save_period > 0 and epoch % opt.save_period == 0:
                    ...
                    logger.log_model(w / f"epoch{epoch}.of")
                del ckpt
...
def parse_opt(known=False):
    ...
    parser.add_argument("--weights", type=str, default=ROOT / "yolov5s-seg.of", help="initial weights path")
...
def main(opt, callbacks=Callbacks()):
    ...
    # Resume
    if opt.resume and not opt.evolve:  # resume from specified or most recent last.of
