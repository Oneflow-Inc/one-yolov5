...
Usage - formats:
    $ python segment/val.py --weights yolov5s-seg.of                 # PyTorch
...
def process_batch(detections, labels, iouv, pred_masks=None, gt_masks=None, overlap=False, masks=False):
    ...
    if masks:
        if overlap:
            nl = len(labels)
            index = flow.arange(nl, device=gt_masks.device).view(nl, 1, 1) + 1
...
def run(
    ...
    metrics = Metrics()
    loss = flow.zeros(4, device=device)
    jdict, stats = [], []
    ...
    for batch_i, (im, targets, paths, shapes, masks) in enumerate(pbar):
        ...
        for si, (pred, proto) in enumerate(zip(preds, protos)):
            ...
            correct_masks = flow.zeros(npr, niou, dtype=flow.bool, device=device)  # init
            correct_bboxes = flow.zeros(npr, niou, dtype=flow.bool, device=device)  # init
            seen += 1

            if npr == 0:
                if nl:
                    stats.append((correct_masks, correct_bboxes, *flow.zeros((2, 0), device=device), labels[:, 0]))
                    if plots:
            ...
            pred_masks = flow.as_tensor(pred_masks, dtype=flow.uint8)
        ...
        if plots and batch_i < 3:
            if len(plot_masks):
                plot_masks = flow.cat(plot_masks, dim=0)
...
def main(opt):
    ...
    if opt.task in ('train', 'val', 'test'):  # run normally
        ...
        opt.half = flow.cuda.is_available() and opt.device != 'cpu'  # FP16 for fastest results
