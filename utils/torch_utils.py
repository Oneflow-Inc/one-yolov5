...
def smart_optimizer(model, name="Adam", lr=0.001, momentum=0.9, decay=1e-5, multi_tensor_optimizer=False):
    ...
    if name == "Adam":
        optimizer = torch.optim.Adam(g[2], lr=lr, betas=(momentum, 0.999), fused=multi_tensor_optimizer)  # adjust beta1 to momentum
    elif name == "AdamW":
        optimizer = torch.optim.AdamW(g[2], lr=lr, betas=(momentum, 0.999), weight_decay=0.0, fused=multi_tensor_optimizer)
    elif name == "RMSProp":
        ...
        optimizer = torch.optim.SGD(g[2], lr=lr, momentum=momentum, nesterov=True, fused=multi_tensor_optimizer)
    else:
...
class ModelEMA:
    ...
    def update(self, model, multi_tensor_optimizer=False):
        ...
        if multi_tensor_optimizer:
            ...
            torch._C.multi_tensor_yolov5_weight_update(weight, weight_update, d)
            cnt = 0
            for k, v in self.ema.state_dict().items():
                if v.dtype.is_floating_point:
                    v = weight[cnt]
