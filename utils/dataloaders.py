...
class LoadStreams:
    ...
    def __init__(
        ...
        flow.backends.cudnn.benchmark = True  # faster for fixed-size inference
        self.mode = "stream"
        self.img_size = img_size
        self.stride = stride
        self.vid_stride = vid_stride  # video frame-rate stride
    ...
    def collate_fn4(batch):
        ...
        for i in range(n):  # zidane flow.zeros(16,3,720,1280)  # BCHW
            ...
            if random.random() < 0.5:
                ...
                im1 = flow.cat(
                    (
                        flow.cat((im[i], im[i + 1]), 1),
                        flow.cat((im[i + 2], im[i + 3]), 1),
                    ),
                    2,
                )
                lb = (
...
class ClassificationDataset(flowvision.datasets.ImageFolder):
    """
    YOLOv5 Classification Dataset.
    Arguments
        root:  Dataset path
        transform:  flowvision transforms, used by default
