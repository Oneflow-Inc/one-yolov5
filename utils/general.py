...
class Profile(contextlib.ContextDecorator):
    ...
    def __init__(self, t=0.0):
        self.t = t
        self.cuda = flow.cuda.is_available()
...
def clip_segments(segments, shape):
    ...
    if isinstance(segments, flow.Tensor):  # faster individually
...
def non_max_suppression(
    ...
    max_nms = 30000  # maximum number of boxes into flowvision.ops.nms()
    ...
    output = [flow.zeros((0, 6 + nm), device=prediction.device)] * bs
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if labels and len(labels[xi]):
            lb = labels[xi]
            v = flow.zeros((len(lb), nc + nm + 5), device=x.device)
            v[:, :4] = lb[:, 1:5]  # box
            v[:, 4] = 1.0  # conf
        ...
        if multi_label:
            ...
            x = flow.cat((box[i], x[i, 5 + j, None], j[:, None].float(), mask[i]), 1)
        else:  # best class only
            conf, j = x[:, 5:mi].max(1, keepdim=True)
            x = flow.cat((box, conf, j.float(), mask), 1)[conf.view(-1) > conf_thres]
...
def apply_classifier(x, model, img, im0):
    ...
    # Example model = flowvision.models.__dict__['efficientnet_b0'](pretrained=True).to(device).eval()
