...
def smart_inference_mode(torch_1_9=check_version(flow.__version__, '1.9.0')):
    # Applies flow.inference_mode() decorator if torch>=1.9.0 else flow.no_grad() decorator
    def decorate(fn):
        return (flow.inference_mode if torch_1_9 else flow.no_grad)()(fn)
...
def smartCrossEntropyLoss(label_smoothing=0.0):
    ...
    if check_version(flow.__version__, '1.10.0'):
...
def smart_DDP(model):
    ...
    assert not check_version(flow.__version__, '1.12.0', pinned=True), \
        'torch==1.12.0 torchvision==0.13.0 DDP training is not supported due to a known issue. ' \
        'Please upgrade or downgrade torch to use DDP. See https://github.com/ultralytics/yolov5/issues/8395'
    if check_version(flow.__version__, '1.11.0'):
...
def select_device(device='', batch_size=0, newline=True):
    ...
    s = f'YOLOv5 🚀 {git_describe() or file_date()} Python-{platform.python_version()} torch-{flow.__version__} '
    ...
    if not cpu and not mps and flow.cuda.is_available():  # prefer GPU if available
        ...
        arg = 'cuda:0'
    elif mps and getattr(torch, 'has_mps', False) and flow.backends.mps.is_available():  # prefer MPS if available
        s += 'MPS\n'
        arg = 'mps'
    else:  # revert to CPU
        s += 'CPU\n'
...
def model_info(model, verbose=False, imgsz=640):
    ...
    try:  # FLOPs
        ...
        im = flow.empty((1, p.shape[1], stride, stride), device=p.device)  # input image in BCHW format
...
def smart_hub_load(repo='ultralytics/yolov5', model='yolov5s', **kwargs):
    # YOLOv5 flow.hub.load() wrapper with smart error/issue handling
    if check_version(flow.__version__, '1.9.1'):
        kwargs['skip_validation'] = True  # validation causes GitHub API rate limit errors
    if check_version(flow.__version__, '1.12.0'):
        kwargs['trust_repo'] = True  # argument required starting in torch 0.12
    try:
        return flow.hub.load(repo, model, **kwargs)
    except Exception:
