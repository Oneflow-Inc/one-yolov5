...
class Annotator:
    ...
    def masks(self, masks, colors, im_gpu, alpha=0.5, retina_masks=False):
        ...
        colors = flow.tensor(colors, device=im_gpu.device, dtype=flow.float32) / 255.0
...
def output_to_target(output, max_det=300):
    ...
    for i, o in enumerate(output):
        ...
        j = flow.full((conf.shape[0], 1), i)
        targets.append(flow.cat((j, cls, xyxy2xywh(box), conf), 1))
    return flow.cat(targets, 0).numpy()
...
def imshow_cls(im, labels=None, pred=None, names=None, nmax=25, verbose=False, f=Path('images.jpg')):
    ...
    blocks = flow.chunk(denormalize(im.cuda().clone()).cpu().float(), len(im),
