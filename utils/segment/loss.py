...
class ComputeLoss:
    ...
    def __call__(self, preds, targets, masks):  # predictions, targets, model
        ...
        lcls = flow.zeros(1, device=self.device)
        lbox = flow.zeros(1, device=self.device)
        lobj = flow.zeros(1, device=self.device)
        lseg = flow.zeros(1, device=self.device)
        ...
        return loss * bs, flow.cat((lbox, lseg, lobj, lcls)).detach()
    ...
    def build_targets(self, p, targets):
        ...
        gain = flow.ones(8, device=self.device)  # normalized to gridspace gain
        ...
        if self.overlap:
            ...
            for i in range(batch):
                ...
                ti.append(flow.arange(num, device=self.device).float().view(1, num).repeat(na, 1) + 1)  # (na, num)
            ti = flow.cat(ti, 1)  # (na, nt)
        else:
            ti = flow.arange(nt, device=self.device).float().view(1, nt).repeat(na, 1)
        targets = flow.cat((targets.repeat(na, 1, 1), ai[..., None], ti[..., None]), 2)  # append anchor indices
        ...
        for i in range(self.nl):
            ...
            tcls.append(c)  # class
            tidxs.append(tidx)
            xywhn.append(flow.cat((gxy, gwh), 1) / gain[2:6])  # xywh normalized
