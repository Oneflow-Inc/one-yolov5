...
class LoadImagesAndLabelsAndMasks(LoadImagesAndLabels):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        masks = (flow.from_numpy(masks) if len(masks) else flow.zeros(1 if self.overlap else nl, img.shape[0] //
        ...
        return (flow.from_numpy(img), labels_out, self.im_files[index], shapes, masks)
    ...
    def collate_fn(batch):
        ...
        batched_masks = flow.cat(masks, 0)
        for i, l in enumerate(label):
            l[:, 0] = i  # add target image index for build_targets()
        return flow.stack(img, 0), flow.cat(label, 0), path, shapes, batched_masks
