...
from utils.flow_utils import (EarlyStopping, ModelEMA, de_parallel, select_device, smart_DDP, smart_optimizer,
                               flow_distributed_zero_first)
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    with flow_distributed_zero_first(LOCAL_RANK):
    ...
    if RANK in {-1, 0}:
        ...
        if not resume:
            ...
            # c = flow.tensor(labels[:, 0])  # classes
            # cf = flow.bincount(c.long(), minlength=nc) + 1.  # frequency
