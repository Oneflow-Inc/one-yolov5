...
def save_one_txt(predn, save_conf, shape, file):
    ...
    for *xyxy, conf, cls in predn.tolist():
        xywh = (
            (xyxy2xywh(flow.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()
        )  # normalized xywh
...
def process_batch(detections, labels, iouv):
    ...
    for i in range(len(iouv)):
        ...
        if x[0].shape[0]:
            matches = (
                flow.cat((flow.stack(x, 1), iou[x[0], x[1]][:, None]), 1).cpu().numpy()
            )  # [label, detect, iou]
            if x[0].shape[0] > 1:
    ...
    if training:  # called by train.py
        ...
        if engine:
            ...
            if not of:
                ...
                LOGGER.info(
                    f"Forcing --batch-size 1 inference (1,3,{imgsz},{imgsz}) for non-OneFlow models"
                )
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        for si, pred in enumerate(out):
            ...
            if npr == 0:
                if nl:
                    stats.append(
                        (correct, *flow.zeros((2, 0), device=device), labels[:, 0])
                    )
                    if plots:
