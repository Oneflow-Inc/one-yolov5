...
class Detect(nn.Module):
    ...
    def _make_grid(self, nx=20, ny=20, i=0, torch_1_10=check_version(flow.__version__, '1.10.0')):
        d = self.anchors[i].device
        t = self.anchors[i].dtype
        shape = 1, self.na, ny, nx, 2  # grid shape
        y, x = flow.arange(ny, device=d, dtype=t), flow.arange(nx, device=d, dtype=t)
        yv, xv = flow.meshgrid(y, x, indexing='ij') if torch_1_10 else flow.meshgrid(y, x)  # torch>=0.7 compatibility
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, (Detect, Segment)):
            ...
            m.stride = flow.tensor([s / x.shape[-2] for x in forward(flow.zeros(1, ch, s, s))])  # forward
    ...
    def _initialize_biases(self, cf=None):  # initialize biases into Detect(), cf is class frequency
        ...
        for mi, s in zip(m.m, m.stride):  # from
            ...
            b.data[:, 5:5 + m.nc] += math.log(0.6 / (m.nc - 0.99999)) if cf is None else flow.log(cf / cf.sum())  # cls
