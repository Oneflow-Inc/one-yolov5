...
from utils.oneflow_utils import (
    fuse_conv_and_bn,
    initialize_weights,
    model_info,
    profile,
...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        self.grid = [flow.empty(0) for _ in range(self.nl)]  # init grid
        self.anchor_grid = [flow.empty(0) for _ in range(self.nl)]  # init anchor grid
        self.register_buffer(
            "anchors", flow.tensor(anchors).float().view(self.nl, -1, 2)
        )  # shape(nl,na,2)
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                if isinstance(self, Segment):  # (boxes + masks)
                    ...
                    y = flow.cat((xy, wh, conf.sigmoid(), mask), 4)
    ...
    def _make_grid(
        self, nx=20, ny=20, i=0, torch_1_10=check_version(flow.__version__, "1.10.0")
    ):
        ...
        yv, xv = (
            flow.meshgrid(y, x, indexing="ij") if torch_1_10 else flow.meshgrid(y, x)
        )  # torch>=0.7 compatibility
        grid = (
            flow.stack((xv, yv), 2).expand(shape) - 0.5
...
class DetectionModel(BaseModel):
    ...
    def __init__(
        ...
        if isinstance(m, (Detect, Segment)):
            ...
            m.stride = flow.tensor(
                [s / x.shape[-2] for x in forward(flow.zeros(1, ch, s, s))]
            )  # forward
...
if __name__ == "__main__":
    ...
    if opt.line_profile:  # profile layer by layer
        ...
        attempt_load('/home/fengwen/datasets/classify/weights/yolov5s.pt')
