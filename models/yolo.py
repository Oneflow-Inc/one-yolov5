        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                mtype = (
                    ...
                    if not isinstance(self, torch.nn.graph.proxy.ProxyModule)
                    else isinstance(self._oneflow_internal_origin__, Segment)
                )# {True Segment , False: Detect}
                if mtype:  # (boxes + masks)
    ...
    def __init__(
        ...
        if isinstance(m, (Detect, Segment)):
            ...
            m.stride = torch.tensor(
                [
                    s / x.shape[-2]
                    for x in process_forward_result(torch.zeros(1, ch, s, s))
                ]
            )  # forward
