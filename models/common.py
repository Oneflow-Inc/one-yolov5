...
from utils.oneflow_utils import copy_attr, smart_inference_mode
...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights='yolov5s.pt', device=flow.device('cpu'), dnn=False, data=None, fp16=False, fuse=True):
        ...
        cuda = flow.cuda.is_available() and device.type != 'cpu'  # use CUDA
        if not (pt or triton):
        ...
        if pt:  # PyTorch
            ...
            model = flow.jit.load(w, _extra_files=extra_files, map_location=device)
            model.half() if fp16 else model.float()
    ...
    def from_numpy(self, x):
        return flow.from_numpy(x).to(self.device) if isinstance(x, np.ndarray) else x
...
class AutoShape(nn.Module):
    ...
    def forward(self, ims, size=640, augment=False, profile=False):
        ...
        #   torch:           = flow.zeros(16,3,320,640)  # BCHW (scaled to size=640, 0-1 values)
...
class Detections:
    ...
    def __init__(self, ims, pred, files, times=(0, 0, 0), names=None, shape=None):
        ...
        gn = [flow.tensor([*(im.shape[i] for i in [1, 0, 1, 0]), 1, 1], device=d) for im in ims]  # normalizations
...
class Classify(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        self.pool = nn.AdaptiveAvgPool2d(output_size=1)  # to x(b,c_,1,1)
