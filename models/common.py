...
class DetectMultiBackend(nn.Module):
    ...
    def warmup(self, imgsz=(1, 3, 640, 640)):
        ...
        if any(warmup_types) and self.device.type != "cpu":
            im = oneflow.zeros(
                *imgsz, dtype=oneflow.half if self.fp16 else oneflow.float, device=self.device
            )  # input
...
class AutoShape(nn.Module):
    ...
    def forward(self, imgs, size=640, augment=False, profile=False):
        ...
        t = [time_sync()]
        p = (
            next(self.model.parameters()) if self.pt else oneflow.zeros(1, device=self.model.device)
        )  # for device, type
...
class Detections:
    ...
    def __init__(self, imgs, pred, files, times=(0, 0, 0, 0), names=None, shape=None):
        ...
        gn = [
            oneflow.tensor([*(im.shape[i] for i in [1, 0, 1, 0]), 1, 1], device=d) for im in imgs
        ]  # normalizations
