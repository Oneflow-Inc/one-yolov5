...
from oneflow_onnx.oneflow2onnx.util import convert_to_onnx_and_check  # noqa :E402
...
def export_torchscript(model, im, file, optimize, prefix=colorstr("TorchScript:")):
    ...
    if optimize:  # https://pytorch.org/tutorials/recipes/mobile_interpreter.html
        LOGGER.info(f"\n error optimize_for_mobile({ts})._save_for_lite_interpreter({str(f)}, _extra_files=f{extra_files})")
    else:
...
def export_onnx(model, im, file, opset, dynamic, simplify, prefix=colorstr("ONNX:")):
    ...
    convert_to_onnx_and_check(yolo_graph, onnx_model_path=str(f), dynamic_batch_size=dynamic, opset=opset, device="gpu" if im.is_cuda else "cpu")
